<?php
// views/pdr/index.php — Objectifs PDR
// Variables injectées par PdrController::index()

// ---- Helpers locaux protégés ----
  ...
  <div class="page-content">
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">Définissez et suivez vos objectifs de vente par vendeur, secteur ou ville</p>
      </div>
    ...
    <?php else: ?>
      ...
      <div class="row g-4 mb-4">
        <?php foreach ($pdrs as $i => $pdr):
          ...
          $taux_global = $pdr['objectif_ca'] > 0 ? $taux_ca : $taux_pieces;
              ...
              <div class="card-body" style="padding:20px;">
                ...
                <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;">
                  <div style="flex:1;min-width:0;">
                    <div style="font-weight:700;font-size:.92rem;margin-bottom:5px;
                ...
                <!-- Dates -->
                <div style="font-size:.74rem;color:var(--text-muted);margin-bottom:14px;display:flex;align-items:center;gap:6px;">
                  <i class="fas fa-calendar-alt" style="color:var(--primary);"></i>
                  <?= format_date($pdr['date_debut']) ?> → <?= format_date($pdr['date_fin']) ?>
                  <?php if ($pdr['jours_restants'] > 0 && $pdr['statut'] === 'en_cours'): ?>
                    &nbsp;·&nbsp;<strong style="color:var(--warning);"><?= $pdr['jours_restants'] ?>j</strong>
                  <?php endif; ?>
                </div>
                  ...
                  <div style="margin-bottom:14px;">
                    <div style="display:flex;justify-content:space-between;margin-bottom:5px;">
                      <span style="font-size:.74rem;font-weight:600;color:var(--text-secondary);">
                        <i class="fas fa-coins"></i> Chiffre d'affaires
                      </span>
                      <span style="font-size:.82rem;font-weight:800;color:<?= pdr_taux_color($taux_ca) ?>;">
                        <?= $taux_ca ?>%
                      </span>
                    </div>
                    <div style="height:10px;background:var(--border);border-radius:99px;overflow:hidden;">
                      <div style="height:10px;width:<?= min($taux_ca, 100) ?>%;background:<?= pdr_taux_color($taux_ca) ?>;border-radius:99px;transition:width 1s ease;"></div>
                    </div>
                    <div style="display:flex;justify-content:space-between;margin-top:5px;font-size:.72rem;color:var(--text-muted);">
                      <span>Réalisé : <strong style="color:var(--text-primary);"><?= format_money($pdr['realise_ca']) ?></strong></span>
                    ...
                    <?php if ($pdr['depassement_ca'] > 0): ?>
                      <div style="margin-top:6px;padding:5px 10px;background:var(--success-soft);border-radius:6px;font-size:.74rem;color:var(--success);font-weight:700;">
                        <i class="fas fa-rocket"></i> Dépassement : +<?= format_money($pdr['depassement_ca']) ?>
                      </div>
                    <?php elseif ($pdr['reste_ca'] > 0 && $pdr['statut'] === 'en_cours'): ?>
                      <div style="margin-top:6px;padding:5px 10px;background:var(--primary-soft);border-radius:6px;font-size:.74rem;color:var(--primary);font-weight:700;">
                        <i class="fas fa-flag-checkered"></i> Reste à faire : <?= format_money($pdr['reste_ca']) ?>
                      </div>
                    <?php endif; ?>
                  </div>
                <?php endif; ?>
                ...
                <?php if ($pdr['objectif_pieces'] > 0): ?>
                  <div style="margin-bottom:14px;">
                    <div style="display:flex;justify-content:space-between;margin-bottom:5px;">
                      ...
                      <span style="font-size:.82rem;font-weight:800;color:<?= pdr_taux_color($taux_pieces) ?>;">
                        <?= $taux_pieces ?>%
                      </span>
                    </div>
                    <div style="height:8px;background:var(--border);border-radius:99px;overflow:hidden;">
                      <div style="height:8px;width:<?= min($taux_pieces, 100) ?>%;background:<?= pdr_taux_color($taux_pieces) ?>;border-radius:99px;"></div>
                    </div>
                    <div style="display:flex;justify-content:space-between;margin-top:5px;font-size:.72rem;color:var(--text-muted);">
                      <span>Réalisé : <strong><?= number_format($pdr['realise_pieces']) ?> pcs</strong></span>
                      <span>Objectif : <strong><?= number_format($pdr['objectif_pieces']) ?> pcs</strong></span>
                    </div>
                    <?php if ($pdr['reste_pieces'] > 0 && $pdr['statut'] === 'en_cours'): ?>
                      <div style="margin-top:5px;font-size:.72rem;color:var(--warning);font-weight:600;">
                        <i class="fas fa-box-open"></i> Reste : <?= number_format($pdr['reste_pieces']) ?> pièces
                      </div>
                    <?php endif; ?>
                  </div>
                <?php endif; ?>
                ...
                <?php if ($pdr['objectif_ventes'] > 0): ?>
                  <div style="display:flex;justify-content:space-between;padding:7px 12px;background:var(--surface-2);border-radius:8px;font-size:.78rem;margin-bottom:10px;">
                    <span style="color:var(--text-secondary);"><i class="fas fa-shopping-cart"></i> Ventes</span>
                    <span><strong><?= $pdr['realise_ventes'] ?></strong> / <?= $pdr['objectif_ventes'] ?></span>
                  </div>
                <?php endif; ?>

                <?php if ($pdr['note']): ?>
                  <div style="font-size:.72rem;color:var(--text-muted);font-style:italic;margin-bottom:10px;padding:6px 10px;background:var(--surface-2);border-radius:6px;">
      ...
      <div class="card fade-up">
        <div class="card-header">
          <div class="card-title"><i class="fas fa-table"></i> Récapitulatif</div>
        </div>
            ...
            <tbody>
              ...
              ?>
                <tr>
                  ...
                  <td style="font-size:.75rem;"><?= format_date($pdr['date_debut']) ?><br><span style="color:var(--text-muted);"><?= format_date($pdr['date_fin']) ?></span></td>
                  <td><?= $pdr['objectif_ca'] > 0 ? format_money($pdr['objectif_ca']) : '—' ?></td>
                  <td><strong><?= format_money($pdr['realise_ca']) ?></strong></td>
                  ...
                  <td><?= number_format($pdr['realise_pieces']) ?> pcs</td>
            ...
            <div class="row g-3">
              ...
              <div class="col-12">
                ...
                <input type="text" name="libelle" id="pdrLibelle" class="form-control"
                  placeholder="Ex : Objectif semaine 20 — Jean Dupont" required>
              </div>

              <div class="col-md-4">
                <label class="form-label">Type de période *</label>
                <select name="type_periode" id="pdrTypePeriode" class="form-control form-select"
                  onchange="autoDateFin()">
              ...
              <div class="col-md-4">
                ...
                <select name="type_cible" id="pdrTypeCible" class="form-control form-select"
                  onchange="toggleCible(this.value)">
                  <option value="global">Global (tous)</option>
                  <option value="vendeur">Un vendeur</option>
                  <option value="secteur">Un secteur</option>
              ...
              <div class="col-md-8" id="cibleContainer" style="display:none;">
                ...
                <select name="cible_id" id="pdrCibleId" class="form-control form-select">
              ...
              <div class="col-12">
                ...
                <div style="font-size:.78rem;font-weight:700;color:var(--text-secondary);text-transform:uppercase;letter-spacing:.06em;">
                  Objectifs à atteindre <span style="font-weight:400;color:var(--text-muted);">(laisser à 0 si non défini)</span>
                </div>
              </div>
              ...
              <div class="col-12">
                ...
                <textarea name="note" id="pdrNote" class="form-control" rows="2"
...
$extra_scripts = <<<JS
<script>
const PDR_VENDEURS = {$vendeursJson};
const PDR_SECTEURS = {$secteursJson};
const PDR_VILLES   = {$villesJson};

function toggleCible(type) {
  const cont  = document.getElementById('cibleContainer');
  const label = document.getElementById('cibleLabel');
  const sel   = document.getElementById('pdrCibleId');
  if (type === 'global') { cont.style.display = 'none'; return; }
  cont.style.display = '';
  const map = { vendeur:[PDR_VENDEURS,'Choisir un vendeur'], secteur:[PDR_SECTEURS,'Choisir un secteur'], ville:[PDR_VILLES,'Choisir une ville'] };
  const [data, lbl] = map[type] || [[], ''];
  label.textContent = lbl;
  sel.innerHTML = '<option value="">-- Sélectionner --</option>';
  data.forEach(item => {
    const o = document.createElement('option'); o.value = item.id; o.textContent = item.nom; sel.appendChild(o);
  });
}
...
function autoDateFin() {
  ...
  document.getElementById('pdrDateFin').value = fin;
}
...
function editPdr(p) {
  ...
  if (p.cible_id) setTimeout(() => { document.getElementById('pdrCibleId').value = p.cible_id; }, 100);
